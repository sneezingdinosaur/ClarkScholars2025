...
Added:
- Prints Top-3 features by importance (per model that supports it).
- CorrelationFilter now tracks kept columns.
- Pipeline order: CorrelationFilter -> preprocessing (impute/scale/var thresh) -> model
  so we can reconstruct final feature names after preprocessing.
"""
...
class CorrelationFilter(BaseEstimator, TransformerMixin):
    def __init__(self, threshold: float = 0.95):
        ...
        self.kept_columns_ = None  # <- track columns we keep (in order)
    ...
    def fit(self, X, y=None):
        ...
        self.kept_columns_ = [c for c in df.columns if c not in self.to_drop_columns_]
        return self

    def transform(self, X):
        df = X if isinstance(X, pd.DataFrame) else pd.DataFrame(X)
        # keep fixed ordering for downstream name alignment
        if self.kept_columns_ is None:
            # fallback (shouldn't happen if fit was called)
            return df.drop(columns=self.to_drop_columns_, errors='ignore')
        return df.loc[:, [c for c in self.kept_columns_ if c in df.columns]]
...
class EnhancedDiabetesPipeline:
    ...
    @staticmethod
    def _extract_feature_importances(fitted_estimator) -> np.ndarray | None:
        """Return a 1D array of importances for the fitted estimator, or None if unsupported."""
        # Tree-based models (RF, XGB, LGBM, GBDT, AdaBoost with trees)
        for attr in ("feature_importances_",):
            if hasattr(fitted_estimator, attr):
                imp = getattr(fitted_estimator, attr)
                if imp is not None:
                    return np.asarray(imp).ravel()

        # Linear models (LogReg): average absolute coef across classes
        if hasattr(fitted_estimator, "coef_"):
            coef = np.asarray(fitted_estimator.coef_)
            if coef.ndim == 1:
                return np.abs(coef)
            # multiclass -> mean abs across classes
            return np.mean(np.abs(coef), axis=0)

        # Not supported (e.g., RBF-SVC, KNN, Voting/Stacking meta)
        return None

    @staticmethod
    def _print_top_features(model_name: str, feature_names: list[str], importances: np.ndarray, topk: int = 3):
        if importances is None or len(importances) != len(feature_names):
            print(f"[{model_name}] Top features: N/A (no importances or mismatch).")
            return
        order = np.argsort(importances)[::-1][:topk]
        print(f"[{model_name}] Top {topk} features by importance:")
    ...
    def train_and_evaluate(self, X, y, random_state: int = 42):
        ...
        # Preprocessing AFTER correlation filtering, so we can track names
        ...
        for name, clf in models_config.items():
            ...
            # -------- NEW: compute and print top-3 feature importances --------
            # Reconstruct final feature names after corr + var threshold
            corr_cols = pipe.named_steps['corr'].kept_columns_ or []
            var_mask = pipe.named_steps['pre'].named_steps['var'].get_support()
            final_feature_names = [c for c, keep in zip(corr_cols, var_mask) if keep]

            importances = self._extract_feature_importances(pipe.named_steps['clf'])
            self._print_top_features(name, final_feature_names, importances, topk=3)
            # ------------------------------------------------------------------
        ...
        stack_pipe = Pipeline([('corr', CorrelationFilter(0.95)), ('pre', clone(numeric_pre)), ('stack', stack)])
        ...
        vote_pipe = Pipeline([('corr', CorrelationFilter(0.95)), ('pre', clone(numeric_pre)), ('vote', vote)])
