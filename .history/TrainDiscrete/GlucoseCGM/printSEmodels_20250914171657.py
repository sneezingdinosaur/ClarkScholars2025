...
Now also prints:
1) A compact table with:
   Class | Model | Accuracy | F1-score (Macro) | AUC-ROC (Macro)
   (all as 10-fold CV mean ± std)

2) At the very end:
   - The base models used by the StackingEnsemble
   - The top meta-features (base-model class probabilities) the stacker’s
     final estimator (LogisticRegression) relies on most.
"""
...
try:
    ...
    # 9a) Print the base models the stacker uses
    print("\nBase estimators used in StackingEnsemble:")
    base_names = [name for name, _ in clf.estimators]  # constructor order
    for name in base_names:
        est = clf.named_estimators_[name]
        print(f" - {name}: {est.__class__.__name__}")

    # 9b) Print top meta-features (which base-model probabilities matter most)
