...
# suppress warnings#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Diabetes‑status prediction – Clinical‑feature edition (no HbA1c, no hyperparameter tuning)
=========================================================================================
...
df_part = pd.read_csv(
    r"C:\Users\sneez\Desktop\OMOPStudyGroup\participants.label_agree.all_three.tsv",
    sep="\t"
)
...
def collapse_group(g):
    ...
    if isinstance(g, str) and g.startswith("insulin"):
        return "diabetic"    
    if isinstance(g, str) and g.startswith("oral"):
        return "diabetic"
...
# 8) Baseline 10‑fold CV
skf = StratifiedKFold(n_splits=10, shuffle=True, random_state=42)
results = {}

print("=== BASELINE RESULTS (10‑Fold CV) ===\n")
for name, pipe in models.items():
    scores = cross_val_score(pipe, X_clean, y_enc, cv=skf, scoring='accuracy', n_jobs=-1)
    preds  = cross_val_predict(pipe, X_clean, y_enc, cv=skf, n_jobs=-1)
    results[name] = {
        'scores': scores,
        'mean': scores.mean(),
        'std': scores.std(),
...
for name, res in results.items():
    print(f"{name:<20}{res['mean']:<12.4f}{res['std']:<10.4f}")
