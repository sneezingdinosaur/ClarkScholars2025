...
def load_clinical(meas_csv, part_tsv):
    ...
    def winsor(df_, k=1.5):
        out = df_.copy()
        for c in out.select_dtypes('number'):
            q1, q3 = out[c].quantile([.25, .75])
            iqr = q3 - q1
            out[c] = out[c].clip(q1 - k*iqr, q3 + k*iqr)
        return out

    df_clean = winsor(df[cols])
...
def main():
    # --- full absolute paths ---
    hrv_csv   = r"/workspace/participant_data_with_hrv.csv"
    meas_csv  = r"/workspace/clean_clinical.csv"
    part_tsv  = r"/workspace/participants_OMOP_HB_Overlap.tsv"
    cgm_csv   = r"/workspace/enhanced_cgm_features_with_labels.csv"
    wear_dir  = r"/workspace/ml_ready_participants"
