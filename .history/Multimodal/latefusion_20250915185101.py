...
# ───────────── 4. MAIN ─────────────
def main():
    hrv_csv="participant_data_with_hrv.csv"
    meas_csv="BESTFILERN.csv"
    part_tsv="/workspace/participants_OMOP_HB_Overlap.tsv"
    cgm_csv="enhanced_cgm_features_with_labels.csv"
    wear_dir="ml_ready_participants"
    ...
    for fold,(tr,te) in enumerate(cv_outer.split(common,y_enc),1):
        ...
        rf_base =Pipeline([('impute',KNNImputer(n_neighbors=5)),
                           ('scale',RobustScaler()),
        ...
        metrics_sv['acc'].append(accuracy_score(y_te,y_pred_sv))
        metrics_sv['f1' ].append(f1_score(y_te,y_pred_sv,average='macro'))
        metrics_sv['auc'].append(roc_auc_score(y_te,avg_proba,multi_class='ovr',average='macro'))
        metrics_sv['ll' ].append(log_loss(y_te,avg_proba))
        ...
        metrics_thr['acc'].append(accuracy_score(y_te,y_pred_thr))
        metrics_thr['f1' ].append(f1_score(y_te,y_pred_thr,average='macro'))
        metrics_thr['auc'].append(roc_auc_score(y_te,avg_proba,multi_class='ovr',average='macro'))
        metrics_thr['ll' ].append(log_loss(y_te,avg_proba))
        ...
        metrics_st['acc'].append(accuracy_score(y_te,y_pred_st))
        metrics_st['f1' ].append(f1_score(y_te,y_pred_st,average='macro'))
        metrics_st['auc'].append(roc_auc_score(y_te,meta_proba,multi_class='ovr',average='macro'))
        metrics_st['ll' ].append(log_loss(y_te,meta_proba))
